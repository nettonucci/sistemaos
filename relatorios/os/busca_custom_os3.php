<?php 
    ...
    $con = open_conexao(); 
    $sql = mysqli_query($con, "SELECT * from os INNER JOIN clientes ON (os.idcliente = clientes.id) WHERE status =".$status);
 
    $sql2 = mysqli_query($con, "SELECT count(*) AS qtd FROM os WHERE status =".$status); 
